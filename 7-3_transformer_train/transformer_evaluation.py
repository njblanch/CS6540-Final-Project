...
def evaluate_model(model, test_loader, criterion, device):
    ...
    for inp in grouped_predictions:
        if len(grouped_predictions[inp]) > 0:  # Avoid division by zero
            rmse_ = np.sqrt(
                mean_squared_error(grouped_targets[inp], grouped_predictions[inp])
            )
...
if __name__ == "__main__":
    ...
    avg_test_loss, mse, rmse, mae, rmse_dist = evaluate_model(
        model, test_loader, criterion, device
    )
    print(
        f"Validation Loss: {avg_test_loss:.4f}, MSE: {mse:.4f}, rMSE: {rmse:.4f}, MAE: {mae:.4f}"
    )
