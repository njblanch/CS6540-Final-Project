"""
Modified model.py to sample a couple of desynchronizations from the data rather than a sliding window of them.
Also consider increasing step size to minimize overlap of audio features.
"""
...
BATCH_SIZE = 32
LEARNING_RATE = 5e-4
SUBSET_SIZE = 0.50  # Proportion of data to use formerly 0.05
        ...
        for video_id in tqdm(self.video_ids, desc="Videos", leave=False):
            ...
            for _, clip in clip_keys.iterrows():
                ...
                for t in range(0, total_frames - self.block_size + 1, self.step_size):
                    ...
                    # Choose N_DRIFTS random drifts from the range
                    random_drifts = random.sample(range(-self.drift_range, self.drift_range + 1), N_DRIFTS)

                    for d in random_drifts:
